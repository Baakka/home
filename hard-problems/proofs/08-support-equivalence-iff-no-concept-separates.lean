...
/-! ## The lattice of the dashboard (Ganter-Wille, via Iordache)

Formal concept analysis gives chapter 3's dashboard an algebra. Over the
context (states, observations, possibility-at), mathlib's complete lattice
of concepts is exactly the discriminative structure of the observation
channel: two states share their observation support precisely when no
concept separates them. Decision-critical ambiguity is then the
acceptable-action partition cutting finer than the concept lattice can. -/
...
/-- States are support-equivalent exactly when no concept of the
observation context separates them: the concept lattice is the complete
algebra of what the dashboard can distinguish. -/
